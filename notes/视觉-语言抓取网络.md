...
```
python train_network.py --dataset grasp-anywhere --dataset-path /home/wjj2080/Datasets/Grasp-Anything --add-file-path /home/wjj2080/Datasets/Grasp-Anything/Grasp-Anything++ --description training_grasp_anything++_lggcnn --use-depth 0 --seen 1 --network lggcnn
```



```
python train_network.py --dataset jacquard --dataset-path /home/wjj2080/Datasets/Jacquard_Dataset --description training_jacquard_ggcnn --use-depth 0 --seen 1 --network ggcnn
```
...
```
python eval.py --gpu-num 1 --dataset_name jacquard --root /home/wjj2080/Datasets/Jacquard_Dataset --ckp_path /home/wjj2080/GraspSAM/final_result/total_vit_t_default/jacquard/2025-12-03-13-15-36/epoch1.pth 
```
...
推算: 如果按 0.9 划分训练集，数量应为: 13064

**grasp-anything++/test/seen.obj:**

列表总长度 (包含的物体ID数量): 573
推算: 如果按 0.9 划分训练集，数量应为: 515

**grasp-anything++/test/unseen.obj:**

列表总长度 (包含的物体ID数量): 230
推算: 如果按 0.9 划分训练集，数量应为: 207
...
因为没有固定种子，每次验证都不一样，上下浮动正负3-4个点都是正常的

## **1.vit_t --seen --validate --save**

2080ti，seen训练，unseen验证，jacquard数据集，epoch_71_sr_**62.30**

然后在seen上验证，**77.8**

这种情况不对应官方任何实验结果，因为官方没有给出mobilesam单独跑jacquard的结果

## 2.vit_t_w_ad --seen --validate --save

```
python train.py --gpu-num 1 --dataset-name grasp_anything --save-start-epoch 20 --root /home/wjj4090/Datasets/Grasp-Anything --sam-encoder-type vit_t_w_ad --seen --validate --save --batch-size 16
```

4090，seen训练，unseen验证，grasp_anything数据集，
